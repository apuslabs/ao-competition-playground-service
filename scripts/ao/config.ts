export const EMBEDDING_PROCESS = 'q7QJTZbOI_avjaltnF-kP21kpL3qCs8sJSP6Wdl9wzM';
export const HERDER_PROCESS = 'J0hmRtTPfV3NFiLvn1Pw9-KqrfeAzRAE2JkKWQjz3t0';
export const POOL_PROCESS = 'HJV6BxQGakxlQe6rfGxOMiifKSFYRz5WEwZOKPLPWEg';
export const EMBEDDING_SERVICE = 'http://127.0.0.1:8082';
export const OLLAMA_PROCESS = '79uAAtUuyh0qjscpzKG51LfNFT_LRDADvBD9Kb6yyks';
export const OLLAMA_SERVICE = 'http://127.0.0.1:11434';
